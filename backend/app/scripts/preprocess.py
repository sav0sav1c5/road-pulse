...
def _encode_municipality(df):
    le = LabelEncoder()
    df['municipality_encoded'] = le.fit_transform(df['municipality'])
...
def _drop_unused_columns(df):
    columns = ['accident_id', 'department', 'longitude', 'latitude']
...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = _encode_municipality(df)
    df = _extract_datetime_features(df)
    df = _encode_involved_vehicles(df)
    df = _encode_description(df)
    df = _encode_target_accident_type()
